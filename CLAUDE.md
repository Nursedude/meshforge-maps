...
- **NodeHistoryDB v2 schema (PR-A 2026-05-22)**: two tables — `nodes_current` (UPSERT per message, one row per node) + `trajectory` (append-only, movement-triggered, per-node cap). A position update writes a trajectory row only when haversine distance from the node's previous trajectory row exceeds `trajectory_move_threshold_meters` (default 50). When the node's row count would exceed `trajectory_rows_per_node` (default 500), the oldest row for that node is deleted in the same transaction. DB size is bounded by (`node_count` × `trajectory_rows_per_node`), not by broker message volume. The v1 `observations` table is gone; analytics queries point at `trajectory`. v1→v2 migration runs once at startup via `PRAGMA user_version` — fresh-start (no backfill of trajectory). The deprecated v1 config keys `node_history_throttle_seconds` / `node_history_heartbeat_seconds` and the matching constructor kwargs were removed in PR-B (2026-05-22); the writer doesn't throttle (UPSERT is cheap) or heartbeat-dedupe (movement is the trigger).
...
- **New SQLite DBs MUST go through `src.utils.db_helpers.connect_tuned`**: vendored from /opt/meshforge after the fleet-host 2026-04-26 wedge (1.95 GB rollback-journal-mode DB stalled the service 16+ minutes in `jbd2_log_wait_commit`). The helper sets WAL + synchronous=NORMAL + journal_size_limit=64MB. Pass `read_only=True` for read-only consumers (e.g. `shared_health_state.py` reading core's `health_state.db`); it opens via the `file:...?mode=ro` URI and skips write-only pragmas. The existing `maps_node_history.db` opens its own connection because `auto_vacuum=INCREMENTAL` must be set BEFORE `journal_mode=WAL` — that hand-tuned ordering is preserved. Anything new starts with `connect_tuned`.
- **node_history disk-full is visible**: `record_observation()` / `record_observations_batch()` classify `sqlite3.OperationalError` — disk-full / I/O errors log at `ERROR` and set `_last_write_error_at`; other sqlite errors log at `WARNING`. `/api/health` surfaces the timestamp + message and deducts 25 points so an out-of-space backlog can't hide behind a green score.
- **NOAA polygon vertex cap**: `MAX_POLYGON_VERTICES = 50_000` in `src/collectors/noaa_alert_collector.py`. Alerts whose Polygon or MultiPolygon coordinate count exceeds the cap are dropped with a `WARNING` naming the alert id — protects the Leaflet renderer from a misbehaving upstream.
- **`aredn_node_ips` config validation**: `MapsConfig.validate_update` rejects entries with `://` (scheme) or spaces and warns on `169.254.x.x` (link-local / cloud IMDS range). The collector concatenates `http://{host}:8080/...`, so a scheme in the entry would otherwise produce a malformed URL silently.
