...
# ---------------------------------------------------------------------------
# File event logging (Step 13)
# ---------------------------------------------------------------------------

from src.utils.event_bus import FileEventLogger, enable_event_logging
...
class TestFileEventLogger:
    def test_writes_jsonl(self, tmp_path):
        ...
        event = NodeEvent.position("!abc", 40.0, -105.0)
        file_logger.log_event(event)

        assert log_path.exists()
        lines = log_path.read_text().strip().split("\n")
        assert len(lines) == 1
        record = json.loads(lines[0])
        assert record["event_type"] == "node.position"
    ...
    def test_multiple_events(self, tmp_path):
        ...
        file_logger = FileEventLogger(log_path=log_path)
        for i in range(5):
            file_logger.log_event(NodeEvent.position(f"!node{i}", float(i), 0.0))
        assert file_logger.events_written == 5
        lines = log_path.read_text().strip().split("\n")
        assert len(lines) == 5
    ...
    def test_rotation_at_size_threshold(self, tmp_path):
        ...
        # Temporarily set a small rotation threshold for testing
        import src.utils.event_bus as eb
        old_max = eb._MAX_LOG_SIZE
        eb._MAX_LOG_SIZE = 500  # 500 bytes

        try:
            for i in range(100):
                file_logger.log_event(
                    NodeEvent.position(f"!node{i:04d}", float(i), float(i))
                )
            rotated = log_path.with_suffix(".jsonl.1")
            assert rotated.exists()
        finally:
            eb._MAX_LOG_SIZE = old_max

    def test_event_serialization_format(self, tmp_path):
        import json
        log_path = tmp_path / "events.jsonl"
        file_logger = FileEventLogger(log_path=log_path)
        event = ServiceEvent.down("meshtastic", reason="timeout")
        file_logger.log_event(event)

        record = json.loads(log_path.read_text().strip())
        assert record["event_type"] == "service.down"
        assert "summary" in record
        assert "timeout" in record["summary"]


class TestEnableEventLogging:
    def test_enable_subscribes_logger(self, tmp_path):
        bus = EventBus()
        log_path = tmp_path / "events.jsonl"
        file_logger = enable_event_logging(bus, log_path=log_path)

        bus.publish(NodeEvent.position("!a", 1.0, 2.0))
        bus.publish(ServiceEvent.up("mqtt"))

        assert file_logger.events_written == 2
        assert log_path.exists()
