...
def _make_app():
    """Create a TuiApp with mocked screen for input handling tests."""
...
def _make_app_with_screen():
    """Create a TuiApp with mocked screen configured for draw tests."""
    from src.tui.app import TuiApp
    app = TuiApp()
    app._running = True
    app._connected = True
...
class TestSafeAddstr:
    ...
    def test_clips_to_window_width(self):
        from src.tui.helpers import safe_addstr
        win = MagicMock()
...
class TestTopologyTab:
    ...
    def test_quality_color_helper(self):
        from src.tui.helpers import _quality_color
        with patch("src.tui.helpers.curses") as mock_curses:
            mock_curses.color_pair.return_value = 42
...
class TestEventsTab:
    ...
    def test_event_log_ring_buffer_truncation(self):
        from src.tui.app import TuiApp
        app = TuiApp()
        # Default maxlen is 500; use a smaller deque to test truncation
        app._event_log = deque(maxlen=10)
        for i in range(20):
    ...
    def test_event_type_color_helper(self):
        from src.tui.helpers import _event_type_color
        with patch("src.tui.helpers.curses") as mock_curses:
